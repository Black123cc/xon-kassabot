...
def callbacks(call):
    ...
    # DEPOSIT tasdiqlash (admin)
        ...
        try:
            ...
            kanal_chek = (
                ...
                f"🆔 User ID: {p['user_id']}\n\n"
                f"ID: {pid[-6:]}\n"
                f"🎰 Platforma: {p.get('platform', '1XBet')}\n"
                f"Berish: {p['amount']:,} 🇺🇿 UZS\n"
                f"Olish: {p['amount']:,} 🇺🇿 {p.get('platform','1XBet').upper()} | ⚡\n\n"
                f"Qabul: 9860256601576083 (OK)\n"
                f"O'tkazma: {p.get('bet_id','')} ID\n\n"
                f"Sana: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            bot.send_message(settings["log_kanal"], kanal_chek, parse_mode="HTML")
        except:
            pass
    ...
    # WITHDRAW tasdiqlash (admin)
        ...
        try:
            ...
            uname2 = f"@{u2['username']}" if u2['username'] else u2['full_name']
            phone2 = u2.get("phone", "Nomalum")
            card = w["card"]
            cf = f"{card[0:4]} {card[4:8]} {card[8:12]} {card[12:16]}" if len(card)==16 else card
            wit_num = ''.join(filter(str.isdigit, wid))[-6:]
            kanal_chek2 = (
                f"✅ Amaliyot muvaffaqiyatli yakunlandi\n\n"
                f"👤 Foydalanuvchi: {uname2}\n"
                f"📱 Tel: {phone2}\n"
                f"🆔 User ID: {w['user_id']}\n\n"
                f"ID: {wit_num}\n"
                f"🎰 Platforma: {w.get('platform','')}\n"
                f"Berish: 🇺🇿 {w.get('platform','').upper()} | ⚡\n"
                f"Olish: 🇺🇿 UZS\n\n"
                f"Qabul: {cf} (OK)\n"
                f"O'tkazma: {w.get('bet_id','')} ID\n\n"
                f"Sana: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            bot.send_message(settings["log_kanal"], kanal_chek2, parse_mode="HTML")
        except:
            pass
